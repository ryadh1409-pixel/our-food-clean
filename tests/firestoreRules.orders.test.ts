...
describe('firestore rules: AI chat food cards', () => {
  it('allows a non-admin user to create an AI food card with its linked HalfOrder in one batch', async () => {
    ...
    batch.set(
      doc(db, 'food_cards', 'ai-card-1'),
      aiChatFoodCard('ai-order-1', 'u1'),
    );
    batch.set(
      doc(db, 'orders', 'ai-order-1'),
      aiChatHalfOrder('ai-card-1', 'u1'),
    );
  ...
  it('denies an AI food card create without the paired linked order', async () => {
    ...
    await assertFails(
      setDoc(
        doc(db, 'food_cards', 'ai-card-2'),
        aiChatFoodCard('missing-order', 'u1'),
      ),
    );
  });
  ...
  it('denies an AI food card when the paired order points at another card', async () => {
    ...
    batch.set(
      doc(db, 'food_cards', 'ai-card-3'),
      aiChatFoodCard('ai-order-3', 'u1'),
    );
    batch.set(
      doc(db, 'orders', 'ai-order-3'),
      aiChatHalfOrder('different-card', 'u1'),
    );
