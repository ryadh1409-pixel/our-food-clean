...
describe('firestore rules: join must preserve existing members', () => {
  it('denies half-order join that replaces the host while growing lists by one', async () => {
    await te().withSecurityRulesDisabled(async (ctx) => {
    ...
    const dbAttacker = te().authenticatedContext('attacker').firestore();
    await assertFails(
  ...
  it('allows honest half-order join that keeps the host and appends the joiner', async () => {
    await te().withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), 'orders', 'hijack2'), {
        cardId: 'fc-hijack-2',
        users: ['host'],
        host: { userId: 'host', name: 'Host', avatar: null, phone: null, expoPushToken: null },
        participants: ['host'],
    ...
    await assertSucceeds(
      updateDoc(doc(dbJoiner, 'orders', 'hijack2'), {
